...
### 13、删除上一次的提交（或删除多次的提交）
第一步：在本地删除提交
```bash
# 如果你只想删除最近的一次提交，可以直接执行：
git rebase -i HEAD~1

# 如果你想删除的提交不在最近一次，可以扩大范围（例如查看最近3次提交）：
git rebase -i HEAD~3

# 修改指令并保存
pick abc1234 你的上次提交说明

# 将你想删除的那次提交前面的 pick 改为 drop（或者直接删除这一整行）：
drop abc1234 你的上次提交说明

# 修改完成后，保存并退出编辑器（如果是 Vim 编辑器，按 Esc 键后输入 :wq 并回车即可）
```

第二步：强制推送到个人远程仓库
```bash
# 因为你在本地修改了提交历史，远程仓库的历史和本地不再一致，所以必须使用强制推送来覆盖远程的记录
